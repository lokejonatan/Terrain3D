...
private:
	...
	std::unordered_map<Vector2i, RegionGpuState, Vector2iHash> _region_color_textures;

	bool _ensure_pipeline();
	void _ensure_fallback_mask();
	RegionGpuState &_get_or_create_region_state(const Vector2i &p_region_loc, Terrain3DRegion *p_region);
